...
class OrderCancellationTest {
    ...
    private final SetmealMapper setmeals=mock(SetmealMapper.class);
    ...
    private final OrderLifecycleService service=new OrderLifecycleService(orders,details,dishes,setmeals,coupons,
    ...
    @BeforeEach void setup() {
        ...
        when(dishes.rollbackStock(1L,2)).thenReturn(1);
    }
    ...
    @Test void paidCancellationQueuesRefundAndRestoresStockOnce() {
        ...
        verify(dishes).rollbackStock(1L,2); verify(coupons).rollbackByOrderId(9L);
        verify(jobs).refund(order,false); verify(jobs).done("timeout:test-cancel");
        service.cancel(9L,"user","USER");
        verify(dishes,times(1)).rollbackStock(1L,2);
    }
    @Test void failedConditionalUpdateCannotRestoreStockOrRefund() {
        ...
        verifyNoInteractions(dishes,coupons,jobs);
    }
