...
/**
 * 餐厅主数据访问。
 * <p>订单在交易库，这里不做订单统计，删除校验时通过 {@code TradeClient} 取。</p>
 */
@Mapper
public interface CanteenMapper {
    ...
    @Select("select count(1) from canteen where campus_zone_id = #{campusZoneId} and name = #{name} "
            + "and id != ifnull(#{excludeId}, 0)")
    int countByZoneAndName(@Param("campusZoneId") Long campusZoneId, @Param("name") String name,
                           @Param("excludeId") Long excludeId);

    @Insert("insert into canteen (campus_zone_id, code, name, location, description, meal_period, status, sort, "
            + "create_time, update_time) values (#{campusZoneId}, #{code}, #{name}, #{location}, #{description}, "
            + "#{mealPeriod}, #{status}, #{sort}, #{createTime}, #{updateTime})")
    void insert(Canteen canteen);

    @Update("update canteen set campus_zone_id=#{campusZoneId}, code=#{code}, name=#{name}, location=#{location}, "
            + "description=#{description}, meal_period=#{mealPeriod}, sort=#{sort}, update_time=#{updateTime} "
            + "where id=#{id}")
